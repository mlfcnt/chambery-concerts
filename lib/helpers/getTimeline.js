...
export const generateTimeline = (concerts, dateFormat) => {
  ...
  const displayDescription = (description) => {
    ...
    if (description.length <= maxDescSize) return <p style={descStyle}>{`${description} [...]`}</p>;
    return <p style={descStyle}>{description.slice(0, maxDescSize)} [...]</p>;
  };
    ...
    for (const {
      ...
      concertsByDate.push(
        <VerticalTimelineElement
          ...
          icon={<CgMusic />}
        >
          <img src={largePicture || tumbnailPicture || defaultPicture} className={styles.image} />
          <h3 className={`vertical-timeline-element-title ${styles.artist}`}>
            {artist.name} {isCanceled && <p style={{ fontSize: '2rem' }}>ANNULÉ</p>}
          </h3>

          <h4 className={`vertical-timeline-element-subtitle ${styles.venue}`}>
          ...
          <SuperCoolButton href={urlDescription} target="_blank" color={backgroundColor}>
