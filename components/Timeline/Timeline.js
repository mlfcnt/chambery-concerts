...
export default function Timeline({ concerts }) {
  const displayConcerts = () => {
    ...
    for (const date in concerts) {
      for (const { startDate, artist, venue, style, description } of concerts[date]) {
        concertsByDate.push(
          <VerticalTimelineElement
            ...
            icon={<CgMusic />}
          >
            <h3 className="vertical-timeline-element-title">Artist : {artist.name}</h3>
            <h4 className="vertical-timeline-element-subtitle">Salle : {venue.name}</h4>
